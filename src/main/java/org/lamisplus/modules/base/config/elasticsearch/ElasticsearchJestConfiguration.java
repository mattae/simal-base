package org.lamisplus.modules.base.config.elasticsearch;
...
@Slf4j
public class ElasticsearchJestConfiguration implements DisposableBean {

    private static final Log logger = LogFactory.getLog(com.github.vanroy.springboot.autoconfigure.data.jest.ElasticsearchJestAutoConfiguration.class);
    ...
    public JestClient testClient() throws NodeValidationException {
        if (!CollectionUtils.isEmpty(properties.getUris()) && StringUtils.hasText(properties.getUris().get(0))) {
    ...
    private JestClient createJestClient(List<String> uris) {
        ...
        if (!CollectionUtils.isEmpty(configBuilderCustomizers)) {
            ...
            configBuilderCustomizers.forEach(customizer -> customizer.customize(builder));
    ...
    private int createInternalNode() throws NodeValidationException {
        ...
        if (logger.isInfoEnabled()) {
            logger.info("Create embedded ES node");
        }
